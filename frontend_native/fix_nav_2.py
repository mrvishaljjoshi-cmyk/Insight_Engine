...
for filename, active_tab in files.items():
    ...
    # Fix broken tags in ledger.html first
    content = content.replace('</main>\n        </div>\n\n        <!-- PROFILE MODAL -->', '</nav>\n    </div>\n\n    <!-- PROFILE MODAL -->')

    # Find the mobile nav block and replace it
    ...
    if re.search(pattern, content):
        ...
        # Some files might not have the </div> right after
        pattern2 = r'<nav class="mobile-nav">[\s\S]*?</nav>'
        if re.search(pattern2, content):
            content = re.sub(pattern2, current_nav.replace('\n    </div>', ''), content)
            with open(f, 'w') as file:
                file.write(content)
            print(f"Updated nav in {f} (pattern 2)")
        else:
